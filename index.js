...
HaxballJS().then((HBInit) => {
    // =====================================
    // ESP League BR | 3v3 - Headless + Firebase Realtime Database
    // =====================================

    var room = HBInit({
        roomName: "🏟️ ESP League BR | 5v5",
        maxPlayers: 10,
        noPlayer: true,
        public: true,
    ...
    // =====================================
    // SISTEMA DE FILA E BALANCEAMENTO OTIMIZADO (SEM RECURSÃO)
    // =====================================
    var fila = [];
    var maxTeamSize = 3; // 3v3
    var gameEnding = false;
    var isManaging = false; // TRAVA DE SEGURANÇA PARA EVITAR TRAVAMENTOS/LOOPS
    var lastKick = null;
    ...
    function manageTeams(winnerTeam) {
        ...
        isManaging = true; // Bloqueia chamadas secundárias enquanto processa

        updateQueue();

        // Quando a partida acaba: move apenas os perdedores para o fim da fila
        ...
        // Se o jogo NÃO está rodando, preenche as vagas
        if (room.getScores() === null) {
            ...
            // Preenche cada time até dar 3v3 de forma equilibrada sem zerar quem já tá jogando
            while (specsInQueue.length > 0 && (currentRed.length < maxTeamSize || currentBlue.length < maxTeamSize)) {
                if (currentRed.length <= currentBlue.length && currentRed.length < maxTeamSize) {
                    var p = specsInQueue.shift();
                    room.setPlayerTeam(p.id, 1);
                    removeFromQueue(p);
                    currentRed.push(p);
                } else if (currentBlue.length < maxTeamSize) {
                    var p = specsInQueue.shift();
            ...
            // Inicia automaticamente apenas se os dois times estiverem com tamanhos iguais e > 0
            if (finalRed > 0 && finalRed === finalBlue) {
                setTimeout(function() {
        ...
        isManaging = false; // Libera o gerenciador
    }
    ...
    room.onPlayerTeamChange = function(changedPlayer, byPlayer) {
        if (isManaging) return; // Ignora o evento se o próprio bot estiver movendo os jogadores!
    ...
    room.onTeamGoal = function(team) {
        if (lastKick && lastKick.team === team) {
            ...
            room.sendAnnouncement("⚽ GOL DE " + lastKick.name + "!", null, 0x00FF00, "bold");
    ...
    room.onPlayerChat = function(player, message) {
        ...
        if (!loggedInPlayers[player.id] && cmd !== "!help") {
            room.sendAnnouncement("⚠️ Você precisa fazer login para interagir! Use: !login <senha>", player.id, 0xFF0000);
            return false;
        }
        ...
        // Inicialização
        loadDB();
        console.log("🚀 Servidor pronto e otimizado sem travamentos!");
})